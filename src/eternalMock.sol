/**
 *Submitted for verification at polygonscan.com on 2024-08-30
 */
...
contract eternal is Context, Ownable, Ieternal {
    ...
    Igenerator private GENERATORCONTRACT; // = Igenerator(0xD829E975B00F4cB4ccd8cEfE80525A219D2d3B15);
    IfanCommunity private FANCONTRACT =
        IfanCommunity(0xDD05ed9e4E14aD0133b285e84eCf836133FA3b7d);
    Iforecast private FORECASTCONTRACT =
        Iforecast(0x12Ba044abC0E6e862A4a0827E03c88568736e867);
    IERC20Metadata private USD;
    /* = IERC20Metadata(0xc2132D05D31c914a87C6611C10748AEb04B58e8F);
     */
    Iinsurance private IC;
    IEIPM private EIPM;

    address private PC;
    address private RADEC;

    bool initiatedRate = false;

    uint256 private growthRate = 19;
    uint256 private growthRateDivisor = 100_000;

    uint256 private _totalSupply;
    uint256 private _totalBurnt;
    uint256 private _totalDonate;

    uint256 private ownerTransferLevy = 10;
    uint256 private ECTransferLevy = 10;
    uint256 private EIPMTransferLevy = 10;

    uint256 private RADSellLevy = 20;
    uint256 private EIPMSellLevy = 20;
    uint256 private ECSellLevy = 20;

    uint256 private _divisor = 1000;

    uint256 private constantRate;
    uint256 private previousRate;
    uint256 private previousRateControl;

    uint256 private rateControlCounter = 0;

    uint256[] private timeStamps;
    uint256[] private ETrates;
    uint256[] private eterRates;

    string private _name = "ETERNAL";
    string private _symbol = "ET";
    string private _description = "";

    constructor(address usdt, address generator) Ownable(msg.sender) {
        USD = IERC20Metadata(usdt);
        GENERATORCONTRACT = Igenerator(generator);
    }
    ...
    function initEternityContracts(
        ...
        RADEC = _RADEC;
        GENERATORCONTRACT = Igenerator(_GENERATORCONTRACT);
        FANCONTRACT = IfanCommunity(_FANCONTRACT);
        FORECASTCONTRACT = Iforecast(_FORECASTCONTRACT);
        IC = Iinsurance(_IC);
        EIPM = IEIPM(_EIPM);
        PC = _PC;

        GENERATORCONTRACT.approve(_IC, type(uint256).max);

        _whitelists[address(this)] = true;
        _whitelists[_IC] = true;
        _whitelists[_EIPM] = true;
        _whitelists[_PC] = true;
        _whitelists[_DC] = true;
        _whitelists[owner()] = true;
        _whitelists[_RADEC] = true;
        _whitelists[ownersDelegate] = true;
    }

    function initRate() external onlyOwner {
        require(!initiatedRate, "EC0");
        constantRate = currentRate();
        previousRate = constantRate;
        previousRateControl = block.timestamp + 3 hours;
    ...
    function eterBalanceOfTreasury() public view returns (uint256) {
        return GENERATORCONTRACT.balanceOf(address(this));
    }

    function eterBalanceOfSecondTreasury() public view returns (uint256) {
        return IC.eterSecondTreasury();
    }
    ...
    function getTotalTransferLevy() public view returns (uint256) {
        return ownerTransferLevy + ECTransferLevy + EIPMTransferLevy;
    }

    function getTotalSellLevy() public view returns (uint256) {
        return RADSellLevy + EIPMSellLevy + ECSellLevy;
    }
    ...
    function getGrowthRate() public view returns (uint256, uint256) {
        return (growthRate, growthRateDivisor);
    }

    function EterToET(uint256 eterAmount) public view returns (uint256) {
        if (_totalSupply == 0 || eterBalanceOfTreasury() == 0)
            return decimalConverter(eterAmount);

        return (eterAmount * _totalSupply) / eterBalanceOfTreasury();
    }

    function ETtoEter(uint256 ETamount) public view returns (uint256) {
        if (_totalSupply == 0 || eterBalanceOfTreasury() == 0)
            return decimalConverter(ETamount);

        return (ETamount * eterBalanceOfTreasury()) / _totalSupply;
    }

    function USDtoEter(uint256 USDamount) public view returns (uint256) {
        return GENERATORCONTRACT.USDtoEter(USDamount);
    }

    function EterToUSD(uint256 amount) public view returns (uint256) {
        return GENERATORCONTRACT.EterToUSD(amount);
    }

    function USDtoET(uint256 USDamount) public view returns (uint256) {
        return EterToET(USDtoEter(USDamount));
    }

    function ETtoUSD(uint256 ETamount) public view returns (uint256) {
        return EterToUSD(ETtoEter(ETamount));
    }

    function currentRate() public view returns (uint256) {
        return ETtoEter(10 ** decimals());
    }

    function rateToReach() public view returns (uint256) {
        uint256 rateToAdd = (constantRate * growthRate) / growthRateDivisor;

        return previousRate + rateToAdd;
    }

    function ETamountRequired() public view returns (uint256) {
        uint256 newRate = rateToReach();
        uint256 newETbalance = (eterBalanceOfTreasury() * (10 ** decimals())) /
            newRate;

        return _totalSupply - newETbalance;
    }

    function eterBalRequired() public view returns (uint256) {
        return (_totalSupply * rateToReach()) / (10 ** decimals());
    }
    ...
    function getRateVariables()
        ...
        returns (uint256, uint256, uint256)
    {
        return (constantRate, previousRate, previousRateControl);
    }
    ...
    function getChangeInRate()
        ...
        returns (uint256[] memory, uint256[] memory, uint256[] memory)
    {
        return (timeStamps, ETrates, eterRates);
    }
    ...
    function buyTutorial() public pure returns (string memory) {
        return
            "In order to purchase ET tokens, visit ETERNITY INSURANCE contract or our DApp at www.eter.to";
    }
    ...
    function controlRate() public {
        require(block.timestamp >= previousRateControl, "EC1");

        uint256 eterBal = eterBalanceOfTreasury();
        uint256 requiredEterBalance = eterBalRequired();

        if (eterBal < requiredEterBalance) {
            uint256 requiredEter = requiredEterBalance - eterBal;

            IC.transferEterToEC(requiredEter);
        ...
        if (rateControlCounter % 8 == 0 && rateControlCounter != 0)
            constantRate = previousRate;

        if (rateControlCounter % 39 == 0) IC.transferToDEFI();

        rateControlCounter++;

        timeStamps.push(block.timestamp);
        ETrates.push(currentRate());
        eterRates.push(
            GENERATORCONTRACT.EterToUSD(10 ** GENERATORCONTRACT.decimals())
        );
    }

    function buy(uint256 eterAmount) public returns (uint256) {
        address caller = _msgSender();

        require(
            caller == address(IC) || caller == PC || caller == address(2),
            "EC2"
        );

        uint256 totalET = EterToET(eterAmount);

        GENERATORCONTRACT.transferFrom(caller, address(this), eterAmount);

        _mint(caller, totalET);
    ...
    function sell(
        uint256 ETamount,
        address receiver
    ) public returns (uint256 eterAmount) {
        address caller = _msgSender();

        if (_totalSupply == ETamount) {
            _burn(caller, ETamount);
            eterAmount = eterBalanceOfTreasury();
            GENERATORCONTRACT.transfer(caller, eterAmount);
            return eterAmount;
        }

        uint256 generatorSellLevy = (ETamount * ECSellLevy) / _divisor;
        uint256 shareLevy = (ETamount * EIPMSellLevy) / _divisor;
        uint256 RandDLevy = (ETamount * RADSellLevy) / _divisor;

        uint256 burnETamount = ETamount - shareLevy - RandDLevy;
        uint256 callersShare = burnETamount - generatorSellLevy;

        eterAmount = ETtoEter(callersShare);

        _burn(caller, burnETamount);
        _update(caller, RADEC, RandDLevy);
        _update(caller, address(EIPM), shareLevy);

        GENERATORCONTRACT.transfer(receiver, eterAmount);

        // controlGrowth();
    }

    function convert(uint256 ETamount) public returns (uint256 eterAmount) {
        address caller = _msgSender();

        require(caller == address(IC) || caller == PC, "EC3");

        eterAmount = ETtoEter(ETamount);
        _burn(caller, ETamount);
        GENERATORCONTRACT.transfer(caller, eterAmount);
    }
    ...
    function donate(uint256 eterAmount) public returns (bool) {
        address caller = _msgSender();
        require(GENERATORCONTRACT.balanceOf(caller) >= eterAmount, "EC4");

        require(
            GENERATORCONTRACT.allowance(caller, address(this)) >= eterAmount,
            "EC5"
        );

        GENERATORCONTRACT.transferFrom(caller, address(this), eterAmount);
        _totalDonate += eterAmount;
    ...
    function controlGrowth() internal {
        uint256 crtRateAfterAction = currentRate();
        uint256 requiredRate = rateToReach();
        uint256 eterBal = eterBalanceOfTreasury();

        if (crtRateAfterAction > requiredRate) {
            uint256 requiredEterBalance = eterBalRequired();

            if (eterBal > requiredEterBalance) {
                uint256 eterToTransfer = eterBal - requiredEterBalance;

                IC.transferEtertoIC(eterToTransfer);
            }
    ...
    function adjustSingleET() internal {
        if (ETamountRequired() == 1 && balanceOf(RADEC) >= 1) {
            _burn(RADEC, 1);
        }
    }

    function decimalConverter(uint256 amount) internal view returns (uint256) {
        return amount * (10 ** (decimals() - GENERATORCONTRACT.decimals()));
    }

    function _transfer(address from, address to, uint256 value) internal {
        if (from == address(0) || to == address(0)) revert("EC6");

        if (value > 0) {
            if (_whitelists[from] || _whitelists[to]) {
                _update(from, to, value);
            } else {
                uint256 crtDivisor = /*EIPM.isHeir(from, to)
                    ? _divisor * 2
                    : */ _divisor;

                uint256 generatoryLevy = (value * ECTransferLevy) / crtDivisor;
                uint256 ownerLevy = (value * ownerTransferLevy) / crtDivisor;
                uint256 shareLevy = (value * EIPMTransferLevy) / crtDivisor;

                _update(from, address(0), generatoryLevy);
                _update(from, owner(), ownerLevy);
                _update(from, address(EIPM), shareLevy);

                _update(
                    from,
                    to,
                    value - ownerLevy - generatoryLevy - shareLevy
                );
